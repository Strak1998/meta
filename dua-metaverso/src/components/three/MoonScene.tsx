...
/* ═══════════════════════════════════════════════════════
   ...
   Runs inside Canvas. Measures actual frame time over a
   rolling 60-frame window and emits quality changes every
   3 seconds to avoid thrashing.
   ═══════════════════════════════════════════════════════ */

function QualityTracker({ onChange }: { onChange: (q: Quality) => void }) {
  const frameTimesRef = useRef<number[]>([]);
  const lastQualityRef = useRef<Quality>("high");
  ...
  useFrame((_, delta) => {
    ...
    const next: Quality = fps < 45 ? "low" : fps < 55 ? "medium" : "high";
    if (next !== lastQualityRef.current) {
...
/* ═══════════════════════════════════════════════════════
   ...
   - All devices:   Bloom + Vignette + ToneMapping (soul of the visual)
   - Non-mobile:    + ChromaticAberration (high only)
   - High quality:  + N8AO + DepthOfField
   - Medium quality: + DepthOfField (reduced bokeh)
   ═══════════════════════════════════════════════════════ */
...
function PostProcessing({
  ...
  /* enableNormalPass is required by N8AO; skip it when N8AO is off to save a render pass */
  const needsNormalPass = quality === "high" && !isMobile;
...
/* ═══════════════════════════════════════════════════════
   ...
   Segments reduced from 128 to 64: identical visual at distance,
   ~75% less vertex data.
   ═══════════════════════════════════════════════════════ */
...
function MassiveMoon() {
  ...
  return (
    <group>
      {/* 64 segments: visually identical to 128 at this distance */}
      ...
      {/* Rings: 128 radial segments (was 256) — imperceptible at ring thickness <0.05 */}
      ...
      {Array.from({ length: 12 }).map((_, i) => {
        ...
        return (
          <mesh
            ...
            position={[Math.sin(a) * 14, 0.008, Math.cos(a) * 14]}
          >
...
function DJBooth() {
  return (
    <group position={[0, 0, -3]}>
      ...
      {[
        {
          pos: [0, 0.68, 0.66] as [number, number, number],
          size: [3.6, 0.01, 0.01] as [number, number, number],
          col: "#00ffcc",
        },
        {
          pos: [0, 0.04, 0.66] as [number, number, number],
          size: [3.6, 0.01, 0.01] as [number, number, number],
          col: "#ff00ff",
        },
        {
          pos: [-1.8, 0.36, 0.66] as [number, number, number],
          size: [0.01, 0.66, 0.01] as [number, number, number],
          col: "#00ffcc",
        },
        {
          pos: [1.8, 0.36, 0.66] as [number, number, number],
      ...
      <mesh position={[0, -0.06, 0]} receiveShadow>
        <cylinderGeometry args={[3.5, 4, 0.12, 48]} />
...
function Turntable({ position }: { position: [number, number, number] }) {
  ...
  useFrame((_, dt) => {
    if (discRef.current) discRef.current.rotation.y += dt * 3.5;
  });
  return (
...
function EQBar({
  ...
  useFrame((s) => {
    if (ref.current) {
      ...
      ref.current.scale.y =
        0.3 +
        Math.max(0, Math.sin(t * 16 + index * 0.65)) *
          2.8 *
  ...
  return (
    <mesh ref={ref} position={position}>
      ...
      <meshBasicMaterial
        color={index < 10 ? "#00ffcc" : index < 15 ? "#ffaa00" : "#ff2244"}
      />
    </mesh>
  );
}
...
/*
  ...
  useGLTF.preload("/models/dj-dua-avatar.glb");
*/
  ...
  useFrame((state) => {
    ...
    if (headRef.current) {
      headRef.current.rotation.x =
        Math.sin(t * 4) * 0.12 + Math.abs(Math.sin(t * 8)) * 0.04;
            ...
            {braids.map((b) => {
              ...
              return (
                <group
                  key={b.i}
                  position={[
                    Math.sin(b.angle) * 0.19,
                    -0.1,
                    Math.cos(b.angle) * 0.17,
                  ]}
                  rotation={[
                    0.15 + b.swayOffset * 0.12,
                    0,
                    b.angle * 0.2,
                  ]}
                >
...
/* ═══════════════════════════════════════════════════════
   GUEST ARTIST AVATAR — reusable, cinematic entrance
   ═══════════════════════════════════════════════════════ */

/*
  GLB MODEL INTEGRATION:
  import { useGLTF } from "@react-three/drei";
  function GuestArtistAvatar({ glbPath, ... }) {
...
function GuestArtistAvatar({
  ...
  useFrame((s) => {
    ...
    if (glowRef.current) {
      const glowIntensity =
        prog < 1 ? 0.5 * (1 - prog) : 0.08 + Math.sin(t * 2) * 0.04;
      (glowRef.current.material as THREE.MeshBasicMaterial).opacity =
        glowIntensity;
  ...
  return (
    <group ref={groupRef}>
      <mesh ref={spotRef} position={[0, 7, 0]}>
      ...
      <mesh position={[0, 1.15, 0]} castShadow>
        <capsuleGeometry args={[0.26, 0.54, 8, 16]} />
        <meshStandardMaterial
          color={shirtColor}
          roughness={0.3}
          metalness={0.5}
      ...
      {[-1, 1].map((side) => (
        <mesh key={`leg-${side}`} position={[side * 0.12, 0.48, 0]} castShadow>
          <capsuleGeometry args={[0.078, 0.6, 4, 8]} />
...
/* ═══════════════════════════════════════════════════════
   AUDIENCE AVATARS — LOD by camera distance
   Near  (<8 units):  full detail + animated arms + name tag
   Mid   (8-16 units): body + head only, no text
   Far   (>16 units):  single sphere (minimal draw call)
   ═══════════════════════════════════════════════════════ */

const SKIN_TONES = [
  "#8d5524", "#c68642", "#e0ac69", "#f1c27d", "#ffdbac",
  "#3b2219", "#4a2c14", "#291711",
];
const SHIRT_COLORS = [
  "#ff0055", "#00ffcc", "#ffffff", "#1a1a1a", "#ffcc00",
  "#8b5cf6", "#3b82f6", "#ef4444", "#ec4899", "#10b981",
];
const FLAGS = ["PT", "CV", "GW", "BR", "MZ", "AO", "ST", "TL"];
const NAMES = [
  "Luna", "DJ_Neon", "Estrela", "Cosmic", "Afro_Beat",
  "VIP_Star", "MoonWalker", "BassHead", "Melodia", "Ritmo",
  "Groove", "Vibe", "Pulse", "Flow", "Wave",
];
...
function AudienceAvatar({
  ...
  /* Groups toggled by LOD distance */
  ...
  useFrame((s) => {
    ...
    if (prog < 1) {
      ...
      ref.current.position.set(
        tx,
        ty + Math.sin(t * 3.5 + idx * 0.5) * 0.14,
        tz
      );
      const faceAngle = Math.atan2(-tx, -(tz + 3));
      ref.current.rotation.y +=
        (faceAngle - ref.current.rotation.y) * 0.05;
      ref.current.rotation.z = Math.sin(t * 2.5 + idx) * 0.1;
    }

    /* LOD: distance-based detail culling */
    const dist = s.camera.position.distanceTo(ref.current.position);
    const isNear = dist < 8;
    const isMid = dist < 16;

    /* Arms only animate when close enough */
    if (armLRef.current && prog >= 1 && isNear) {
      armLRef.current.rotation.x =
        Math.sin(t * 4.2 + idx) * 0.5 + 0.3;
      armLRef.current.rotation.z =
        Math.sin(t * 2 + idx) * 0.2 + 0.5;
    }
    if (armRRef.current && prog >= 1 && isNear) {
      armRRef.current.rotation.x =
        Math.sin(t * 3.8 + idx + 1) * 0.5 + 0.3;
      armRRef.current.rotation.z =
        Math.sin(t * 2.3 + idx + 1) * -0.2 - 0.5;
    }

    /* Hide arms and label beyond thresholds */
    if (fullDetailRef.current) fullDetailRef.current.visible = isMid;
    if (labelRef.current) labelRef.current.visible = isNear;
  });

  return (
    <group ref={ref}>
      {/* Body + head — always visible when avatar is active */}
      <mesh castShadow position={[0, 0.42, 0]}>
        <capsuleGeometry args={[0.15, 0.34, 4, 8]} />
      ...
      {/* Medium + near detail: arms, legs, mic */}
      <group ref={fullDetailRef}>
        <mesh
          ref={armLRef}
          position={[-0.22, 0.62, 0.06]}
          rotation={[0.3, 0, 0.5]}
        >
          <capsuleGeometry args={[0.042, 0.3, 4, 6]} />
          <meshStandardMaterial color={skin} roughness={0.45} />
        </mesh>
        <mesh
          ref={armRRef}
        ...
        <mesh position={[-0.07, 0.1, 0]}>
          <capsuleGeometry args={[0.042, 0.22, 4, 6]} />
        ...
        >
          ...
          <meshBasicMaterial
            color={idx % 2 === 0 ? "#00ffcc" : "#ff00ff"}
          />
        </mesh>
      </group>

      {/* Near-only: name label */}
      <group ref={labelRef}>
...
function AudienceCrowd({
  ...
  const avatars = useMemo(() => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const angle =
        ((col - (totalInRow - 1) / 2) / totalInRow) * Math.PI * 0.9 +
        (((i * 7 + 3) % 11) / 11) * 0.06;
      const radius =
        5.5 + row * 1.9 + (((i * 13 + 5) % 9) / 9) * 0.6;
      result.push({
        target: [
...
function CinematicLighting({ phase }: { phase: ConcertPhase }) {
  ...
  useFrame((s) => {
    ...
    if (strobeRef.current) {
      strobeRef.current.intensity =
        Math.sin(t * 20) > 0.94 ? 10 * intensity : 0;
    }
  });
    ...
    <>
        ...
        {Array.from({ length: 10 }).map((_, i) => {
          ...
          return (
            <mesh
              ...
              position={[Math.sin(a) * 1.2, -3, Math.cos(a) * 1.2]}
            >
              <cylinderGeometry args={[0.008, 2.5, 24, 12, 1, true]} />
              <meshBasicMaterial
      ...
      {[-2, -0.5, 0.5, 2].map((x, i) => (
        <mesh
          key={`godray-${i}`}
          position={[x * 2, 6, -8]}
          rotation={[0.12, 0, x * 0.15]}
        >
          <planeGeometry args={[0.35, 16]} />
          <meshBasicMaterial
            color={i % 2 === 0 ? "#00ffcc" : "#8844ff"}
            transparent
            opacity={0.01}
            blending={THREE.AdditiveBlending}
            depthWrite={false}
...
/* ═══════════════════════════════════════════════════════
   FLOATING PARTICLES
   Group rotation instead of per-frame buffer mutation:
   eliminates O(n) JS work on the main thread every frame.
   ═══════════════════════════════════════════════════════ */

function FloatingParticles({ quality }: { quality: Quality }) {
  const ref = useRef<THREE.Points>(null);
  const count = quality === "high" ? 300 : quality === "medium" ? 150 : 80;
  ...
  useFrame((s) => {
    if (ref.current) {
      /* Simple rotation — zero JS work vs. previous per-vertex mutation */
      ref.current.rotation.y = s.clock.elapsedTime * 0.012;
      ref.current.rotation.x =
        Math.sin(s.clock.elapsedTime * 0.007) * 0.05;
    }
  });
...
export default function MoonScene({
  ...
  /* Mobile starts at medium quality; degrades further if FPS drops */
  const effectiveQuality: Quality = isMobile
    ? quality === "high" ? "medium" : quality
    : quality;

  const showDUA2Screen =
    concertPhase === "dua2_presentation" || concertPhase === "finale";
  const showVado =
    concertPhase === "vado_performance" || concertPhase === "finale";
  const showUzzy =
    concertPhase === "uzzy_performance" || concertPhase === "finale";
  const showEstraca =
    concertPhase === "estraca_performance" || concertPhase === "finale";

  /* Sparkle counts scale with quality */
  const sparkleMain =
    effectiveQuality === "high" ? 400 : effectiveQuality === "medium" ? 200 : 80;
  const sparkleMid =
    effectiveQuality === "high" ? 180 : effectiveQuality === "medium" ? 80 : 0;
  const sparkleAccent =
    effectiveQuality === "high" ? 80 : effectiveQuality === "medium" ? 30 : 0;
    ...
    <div className="absolute inset-0 w-full h-full bg-[#030305]">
      <Canvas
        ...
        onCreated={({ gl }) => {
          ...
          /* Disable shadows on mobile to reclaim GPU budget */
      ...
      >
        ...
        {/* Adaptive quality tracker — invisible component inside Canvas */}
        <QualityTracker onChange={setQuality} />
        ...
        <Suspense fallback={null}>
          ...
          <Stars
            radius={140}
            depth={80}
            count={effectiveQuality === "low" ? 3000 : 8000}
            factor={5.5}
            saturation={0.12}
            fade
            speed={0.5}
          ...
          {artists
            ? artists.filter((a) => a.name && (a.status === "em_palco" || a.status === "pronto")).map((a) => (
                <GuestArtistAvatar
                  key={a.id}
                  name={a.name}
                  active={a.status === "em_palco"}
                  position={POSITION_MAP[a.defaultPosition] ?? [-4, 0, -1]}
                  skinColor={a.skinColor}
                  shirtColor={a.clothingColor}
                  accentColor={a.accentColor}
                  entranceSide={a.defaultPosition === "direita" ? "right" : "left"}
                />
              ))
            : (
              <>
          ...
          />
          <FloatingParticles quality={effectiveQuality} />
